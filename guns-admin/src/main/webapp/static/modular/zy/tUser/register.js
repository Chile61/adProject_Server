 ...
 * 清除数据
 */
reUserInfo.clearData = function() {
    reUserInfo.reUserInfoData = {};
}
/**
 * 收集数据
 */
reUserInfo.collectData = function() {
    reUserInfo
        .set('account')
        .set('password')
        .set('phone')
 ...
 * @param val 数据的具体值
 */
reUserInfo.set = function(key, val) {
    reUserInfo.reUserInfoData[key] = (typeof val == "undefined") ? $("#" + key).val() : val;
    return reUserInfo;
}
...
var regUsername = /^[a-zA-Z0-9_-]{4,19}$/;
...
$('.container').find('input').eq(0).change(function () {
    ...
    if (regUsername.test($(this).val())) {
        ...
        fail($(this), 0, '用户名只能为字母，数字，下划线，减号')
    }
...
$('.container').find('input').eq(1).change(function () {
    ...
    var checkpassword = $('.container').find('input').eq(2)
    if (checkpassword.val() == password && checkpassword.val() != null) {
        success($(checkpassword), 2);
    } else {
        fail($(checkpassword), 2, '两次输入的密码不一致');
    }
});
...
$('.container').find('input').eq(2).change(function () {
    ...
    if ($(this).val() == password && $(this).val() != null) {
        success($(this), 2);
    } else {
...
$('#loadingButton').click(function () {
    ...
    if (check[4]) {
        ...
        $('.container').find('input:visible').eq(4).parent().parent().removeClass('has-success').addClass('has-error');
    }
...
$('#submit').click(function (e) {
    if (!check.every(function (value) {
        ...
        for (key in check) {
            if (!check[key]) {
                $('.container').find('div').find('input').eq(key).parent().parent().removeClass('has-success').addClass('has-error')
            }
        }
        reUserInfo.clearData();
        reUserInfo.collectData();

        //提交信息
        var ajax = new $ax(Feng.ctxPath + "/tUser/webregister", function (data) {Feng.success(data);}, function (data) {Feng.error("注册失败!" + data.responseJSON.message + "!");});
        ajax.set(reUserInfo.reUserInfoData);
        ajax.start();
    }
});
